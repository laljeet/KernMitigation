...
#
# MAIN PAPER:
#   Table 1: Sobol sensitivity indices
#
# SUPPLEMENTARY:
#   Table S1: scenario design (all 25 scenarios)
#   Table S2: headline results, spread sweep + clustered families
#   Table S3: multi-year coupling, year-level trajectory
#   Table S4: contractor queue sensitivity
#
...
fmt_headline <- function(d) {
  d %>% transmute(
    ...
    `Within 365 d (%)` = sprintf("%.0f", mean_pct_resolved_within_365_days * 100),
...
# =====================================================================
# MAIN TABLE 1: Sobol sensitivity indices
# =====================================================================
pretty_sobol <- c(
...
write_csv(t1, file.path(tdir_main, "table1_sobol.csv"))
cat("Main Table 1 (Sobol) written.\n")
...
cat("Supplementary Table S1 (design) written.\n")

# =====================================================================
# SUPPLEMENTARY S2: Headline results, spread sweep + clustered
# =====================================================================
...
s2 <- s2_src %>%
  fmt_headline() %>%
  mutate(Family = s2_src$Family, .before = 1)
write_csv(s2, file.path(tdir_supp, "tableS2_headline_results.csv"))
cat("Supplementary Table S2 (headline) written.\n")

# =====================================================================
# SUPPLEMENTARY S3: Multi-year coupling, year-level trajectory
# =====================================================================
...
write_csv(s3, file.path(tdir_supp, "tableS3_multiyear.csv"))
cat("Supplementary Table S3 (multi-year) written.\n")

# =====================================================================
# SUPPLEMENTARY S4: Contractor queue sensitivity
# =====================================================================
cq_slots <- tibble(
  scenario_id = sprintf("CQ%02d", 1:12),
...
write_csv(s4, file.path(tdir_supp, "tableS4_contractor_queue.csv"))
cat("Supplementary Table S4 (contractor queue) written.\n")
